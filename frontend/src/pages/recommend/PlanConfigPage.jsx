  ...
  async function handleGeneratePlan() {
    ...
    try {
      const data = {
        ...
        snacks_per_day: parseInt(snacksPerDay, 10),
        preferences: {
          vegan: Boolean(vegan),
          vegetarian: Boolean(vegetarian),
          cuisine: cuisine.trim() ? cuisine.trim() : null,
          allergies: parseCommaSeparatedList(allergiesText),
          disliked_ingredients: parseCommaSeparatedList(dislikedText),
        },
        predicted_burn: Number(prediction.total_calories),
      }

      const response = await api.post('/recommend/generate-plan', data)
      try {
        const c = cuisine.trim()
        if (c) localStorage.setItem(LAST_CUISINE_KEY, c)
        else localStorage.removeItem(LAST_CUISINE_KEY)
      } catch {
        /* ignore storage */
      }
      navigate('/recommend/plan', {
        state: { mealPlan: response.data, prediction },
      })
    } catch (err) {
      ...
      <div className="min-h-svh bg-slate-50">
        ...
        <main className="mx-auto max-w-3xl px-4 py-10">
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-6">
            <h1 className="text-lg font-semibold text-amber-900">No workout summary found</h1>
            <p className="mt-2 text-sm text-amber-800">
              Please complete your workout summary before configuring your nutrition plan.
            </p>
            <button
              type="button"
  ...
  return (
    <div className="min-h-svh bg-slate-50">
      {planLoading && (
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-2xl">
            <Loader2 className="mx-auto h-12 w-12 animate-spin text-primary-500" aria-hidden />
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:py-10">
        <section className="rounded-xl border border-slate-200/90 bg-white p-8 shadow-lg ring-1 ring-slate-900/5">
          <div className="border-b border-slate-100 pb-6">
            <p className="text-xs font-medium tracking-wide text-slate-400">Continue</p>
            <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-900">
              Personalize Your Daily Nutrition
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-slate-600">
              We&apos;ve calculated your daily needs based on your profile and recent activity. This
              session adds about{' '}
          ...
          <div className="mt-8 space-y-10">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Daily structure</h2>
              <p className="mt-1 text-xs text-slate-500">
                How many main meals and snacks should each day include?
              </p>
              <div className="mt-4 grid gap-6 sm:grid-cols-2">
                <div>
                  <label htmlFor="meals-count" className="block text-sm font-medium text-slate-800">
                    Meals per day
                  </label>
                ...
                <div>
                  <label htmlFor="snacks-count" className="block text-sm font-medium text-slate-800">
                    Snacks per day
                  </label>
            ...
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Dietary preferences</h2>
              <p className="mt-1 text-xs text-slate-500">
            ...
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Cuisine</h2>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-3 border-t border-slate-100 pt-8">
            ...
            <button
              type="button"
              onClick={() => navigate('/workout/summary', { state: { prediction } })}
